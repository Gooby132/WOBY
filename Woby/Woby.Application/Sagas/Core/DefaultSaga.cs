...
internal class DefaultSaga<InputMessage> : ISaga<InputMessage>, ISagaTransmitter
{
    ...
    public async Task ReceiveMessage(Stream inputStream, NetworkMetadata metadata)
    {
        ...
        var common = converter.Convert(parsed.Value, metadata);
    ...
    public async Task<Result> SendMessage(MessageBase message)
    {
        ...
        if(message is Trying)
        {
            outputStream = await builder.BuildTrying(message);
        }

        if (message is Ringing)
        {
            outputStream = await builder.BuildRinging(message);
        }
